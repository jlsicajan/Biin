<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Biin Transporte Publico</title>
    <meta name="description"
          content="A free responsive website template made exclusively for Frittt by Themesforce and Sarfraz Shaukat">
    <meta name="keywords" content="website template, css3, one page, bootstrap, app template, web app, start-up">
    <meta name="author" content="Themesforce and Sarfraz Shaukat for Frittt">
    <link rel="icon" type="image/ico" href="assets/pics/biin_logo.png"/>
    <link rel="stylesheet" href="new_template_assets/scss/css/bootstrap.css">
    <link rel="stylesheet" href="new_template_assets/fonts/font-awesome-4.3.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="new_template_assets/scss/css/all.css">
    <link rel="stylesheet" href="assets/css/flickity.min.css">
    <link rel="stylesheet" href="assets/css/testimonial.css">
    <link href='http://fonts.googleapis.com/css?family=Montserrat:400,700|Source+Sans+Pro:400,700,400italic,700italic'
          rel='stylesheet' type='text/css'>

    <!-- Google Tag Manager -->
    <script>(function (w, d, s, l, i) {
            w[l] = w[l] || [];
            w[l].push({
                'gtm.start':
                    new Date().getTime(), event: 'gtm.js'
            });
            var f = d.getElementsByTagName(s)[0],
                j = d.createElement(s), dl = l != 'dataLayer' ? '&l=' + l : '';
            j.async = true;
            j.src =
                'https://www.googletagmanager.com/gtm.js?id=' + i + dl;
            f.parentNode.insertBefore(j, f);
        })(window, document, 'script', 'dataLayer', 'GTM-PWWRSQ9');</script>
    <!-- End Google Tag Manager -->
    <title>Biin Transporte Publico</title>
    <!-- this is the code that will allow icon to be visible in the title bar of the brower -->
    <link rel="icon" type="image/ico" href="assets/pics/biin_logo.png"/>
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-127468802-1"></script>
    <!--Start of Zendesk Chat Script-->
    <script lang="es" type="text/javascript">
        window.$zopim || (function (d, s) {
            var z = $zopim = function (c) {
                z._.push(c)
            }, $ = z.s =
                d.createElement(s), e = d.getElementsByTagName(s)[0];
            z.set = function (o) {
                z.set._.push(o)
            };
            z._ = [];
            z.set._ = [];
            $.async = !0;
            $.setAttribute("charset", "utf-8");
            $.src = "https://v2.zopim.com/?65WpZfrXS7ZUJWjJe3pPVPzsiFsDyhKT";
            z.t = +new Date;
            $.type = "text/javascript";
            e.parentNode.insertBefore($, e)
        })(document, "script");
    </script>
    <!--End of Zendesk Chat Script-->
</head>
<body>
<div id="wrapper">
    <header id="header">
        <div class="container">
            <div class="logo">
                <a href="."><img class="biin_logo" src="assets/pics/biin_logo.png" alt="BiiN"></a>
            </div>
            <nav id="nav">
                <div class="opener-holder">
                    <a href="#" class="nav-opener"><span></span></a>
                </div>
                <a href="http://get.biin.mx" class="btn btn-primary rounded">Descarga!!</a>
                <div class="nav-drop">
                    <ul>
                        <li><a class="text-white" href="http://biin.mx/comunidad.html">Comunidad Biin</a></li>
                        <li><a class="text-white" href="http://blog.biin.mx">BiiN Blog</a></li>
                        <li><a class="text-white" href="http://comunidad.biin.mx">Foro BiiN</a></li>
                        <li><a class="text-white" href="http://biin.mx/cuentanos.html">Contacto</a></li>
                    </ul>
                    <div class="drop-holder visible-sm visible-xs">
                        <span>Siguenos</span>
                        <ul class="social-networks">
                            <li><a href="http://www.facebook.com/biinmex"><i class="fa fa-facebook-f"></i></a></li>
                            <li><a href="https://www.instagram.com/biinmex"><i class="fa fa-instagram"></i></a></li>
                            <li><a href="http://www.snapchat.com/add/biinmex"><i class="fa fa-snapchat"></i></a></li>
                            <li><a href="http://www.pinterest.com/biinmx"><i class="fa fa-pinterest"></i></a></li>
                            <li><a href="http://www.twitter.com/biinmex"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="http://www.linkedin.com/company/biinmx"><i class="fa fa-linkedin"></i></a></li>
                        </ul>
                    </div>
                </div>
            </nav>
        </div>
    </header>
    <section class="visual">
        <div class="container">
            <div class="text-block">
                <div class="heading-holder">
                    <h1>BiiN</h1>
                </div>
                <p class="tagline text-biin-white main_p_subtitle">
                    <strong class="main-background main_subtitle">Modernízate ahora y olvídate de las monedas</strong>
                </p>
                <span class="info text-biin-white main_third_title">
                    <strong>PAGA EL TRASPORTE PÚBLICO CON TU CELULAR</strong>
                </span>
            </div>
        </div>
        <img src="assets/pics/first_banner.jpg" alt="Image 1" class="w3-image main_header_image">
    </section>
    <section class="main">
        <div class="container">
            <div id="cta" class="row">
                <a href="http://get.biin.mx" class="btn btn-primary rounded">Descarga la app</a>
                <p>Ayuda a mejorar el transporte y olvidate de las monedas!</p>
            </div>
            <div class="row d-flex align-items-center justify-center flex-sm-column">
                <div class="text-box col-md-12 col-sm-12">
                    <h1 class="w3-jumbo">
                        <font color="black"><b>BiiN</b></font>
                        <img src="assets/pics/biin_logo.png" alt="Biin" style="width:100px;height:100px;">
                    </h1>
                    <h2><font color="red">Reinventando el transporte Público!</font></h2>
                </div>
                <div class="text-box col-md-12 col-sm-12">
                    <p>Paga el transporte público con tu celular - Descarga nuestra aplicación,
                        acerca tu celular al lector y olvidate de las monedas.</p>
                    <a class="l-height-15" href="http://get.biin.mx" target="_blank">
                        <img src="assets/pics/googleplay.png" alt="Descarga" style="width:120px;height:42px;border:0;">
                    </a>
                    <p class="l-height-15">Disponible Para: <i class="fa fa-android w3-xlarge w3-text-green"></i></p>
                </div>
            </div>
        </div>
    </section>
    <section class="area">
        <div class="container">
            <div class="row">
                <h2 class="text-main text-center how_it_works">¿Cómo funciona?</h2>
                <div class="col-sm-12 col-md-6 d-flex align-items-center justify-center flex-column how_it_works_items">
                    <div class="row">
                        <div class="col-md-4 col-sm-3 col-xs-5"><img src="assets/pics/downloads.png" width="110" alt="">
                        </div>
                        <div class="col-md-8 col-sm-9 col-xs-7">
                            <h3><strong>Descarga</strong></h3>
                            <p>Descarga la aplicacion desde Google Play o de la Appstore. </p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4 col-sm-3 col-xs-5"><img class="pull-left" src="assets/pics/registra.png"
                                                                     width="105" alt=""></div>
                        <div class="col-md-8 col-sm-9 col-xs-7">
                            <h3><strong>Registrate</strong></h3>
                            <p>Registrate en la aplicación. </p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4 col-sm-3 col-xs-5"><img src="assets/pics/NFC%20icon.png" height="105"
                                                                     alt=""></div>
                        <div class="col-md-8 col-sm-9 col-xs-7">
                            <h3><strong>Acerca tu celular</strong></h3>
                            <p>Acerca tu celular al TAG en el autobus y espera unos segundos. </p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4 col-sm-3 col-xs-5"><img src="assets/pics/validate.png" height="105" alt="">
                        </div>
                        <div class="col-md-8 col-sm-9 col-xs-7">
                            <h3><strong>Confirma</strong></h3>
                            <p>Confirma tu viaje al conductor. </p>
                        </div>
                    </div>
                </div>
                <div class="col-sm-12 col-md-6 d-flex justify-center">
                    <img src="new_template_assets/images/app_screenshots.gif" width="80%" height="60%"
                         alt="app">
                </div>
            </div>
        </div>
    </section>

    <section class="visual-container area">
        <div class="visual-area">
            <div class="container">
                <h2 class="text-main what_our_customers_say">Que dicen nuestros clientes</h2>
                <div class="main-gallery">
                    <div class="gallery-cell">
                        <div class="testimonial">
                            <img class="testimonial-avatar" src="assets/pics/luis.jpg">
                            <q class="testimonial-quote text-dark">Ya era momento de innovación en el transporte
                                publico!!
                                Esta aplicación a demas de hacernos la vida mas facil recolecta datos para mejorar el
                                tranporte.</q>
                            <span class="testimonial-author text-dark">Miguel Lopez</span>
                        </div>
                    </div>
                    <div class="gallery-cell">
                        <div class="testimonial">
                            <img class="testimonial-avatar" src="assets/pics/itzel.jpg">
                            <q class="testimonial-quote text-dark">Lo mejor... esta app mejora la experiencia de viajar
                                en transporte público</q>
                            <span class="testimonial-author text-dark">Itzel De la Rosa</span>
                        </div>
                    </div>
                    <div class="gallery-cell is-selected">
                        <div class="testimonial">
                            <img class="testimonial-avatar" src="assets/pics/jose.jpg">
                            <q class="testimonial-quote text-dark">Odio las monedas, gracias por esta innovación... Esta
                                super sencillo de utilizar, sugerire a mis amigos y conocidos la utilicen para mejorar
                                día con día el transporte...</q>
                            <span class="testimonial-author text-dark">Jose Mier</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="visual-container">
        <div class="visual-area">
            <div class="container">
                <div class="row">
                    <h2 class="text-main know_us">Conócenos</h2>
                    <div class="col-md-12">
                        <video class="know_us_video" width="100%" controls
                               onclick="this.paused ? this.play() : this.pause();">
                            <source src="new_template_assets/videos/biinvideo.mp4" type="video/mp4">
                            Your browser does not support HTML5 video.
                        </video>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="area subscribe_area">
        <div class="container">
            <div class="row">
                <div class="col-md-12 col-lg-6 col-sm-12">
                    <h3 class="text-main"><strong>Suscríbete a nuestros comunicados</strong></h3>
                </div>
                <div class="col-md-12 col-lg-6 col-sm-12">
                    <form class="form-inline" id="subscribe_to" autocomplete="off">
                        <button type="submit" class="btn btn-primary rounded suscribe subscribe_btn">Suscribirme
                        </button>
                        <div class="form-group">
                            <input type="email" class="form-control rounded email_to_subscribe" id="email_to_subscribe"
                                   placeholder="Correo electronico" required="required">
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </section>

    <footer id="footer">
        <div class="container">
            <div class="footer-holder">
                <div class="row text-center">
                    <div class="col-md-12">
                        <a href="#"><img src="assets/pics/biin_logo.png" alt="Sports HTML Template"></a>
                    </div>

                </div>
                <div class="row">
                    <div id="cta" class="cta-footer">
                        <a href="http://get.biin.mx" class="btn btn-primary rounded">Descarga el app</a>
                        <p>Tu puede ayudar a cambiar el transporte publico.</p>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <h3>El transporte publico es de todos</h3>
                        <br>
                        <a href="http://www.biin.mx/terminosycondiciones.html" target="_blank"><p class="w3-medium">
                                Términos y condiciones</p></a>
                        <a href="http://www.biin.mx/aviso-privacidad.html" target="_blank"><p class="w3-medium">
                                Políticas de privacidad</p></a>
                        <a href="http://blog.biin.mx" target="_blank"><p class="w3-medium">BiiN Blog</p></a>
                        <a href="http://comunidad.biin.mx" target="_blank"><p class="w3-medium">Comunidad BiiN</p></a>

                    </div>

                    <div class="col-md-6 text-right">
                        <ul class="social">
                            <li><a href="http://www.facebook.com/biinmex"><i class="fa fa-facebook-f"></i></a></li>
                            <li><a href="https://www.instagram.com/biinmex"><i class="fa fa-instagram"></i></a></li>
                            <li><a href="https://www.youtube.com/channel/UC3CMXs1tWtD9-y02zz5W26w"><i
                                        class="fa fa-youtube"></i></a></li>
                            <li><a href="http://www.twitter.com/biinmex"><i class="fa fa-twitter"></i></a></li>
                            <li><a href="http://www.linkedin.com/company/biinmx"><i class="fa fa-linkedin"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <div class="row text-center">
            <p>Powered by <a href="https://www.biin.mx" target="_blank" class="w3-hover-text-green">Biin
                    Science SAPI de CV</a></p>
        </div>
    </footer>
</div>
<div class="modal_container hide">
    <div class="modal_subscribe">
        <div class="row">
            <div class="col-md-12">
                <div class="close_modal_subscribe">
                    <span id="x">X</span>
                </div>
                <div class="modal-body">
                    <h2 class="text-main"><strong>Gracias por suscribirte.</strong></h2>
                    <h2 class="text-main"><strong>Pronto recibirás nuestros comunicados.</strong></h2>
                </div>
            </div>
        </div>
        <div class="modal_background"></div>
    </div>
</div>
<script src="new_template_assets/js/jquery-1.11.2.min.js"></script>
<script src="new_template_assets/js/bootstrap.js"></script>
<script src="new_template_assets/js/jquery.main.js"></script>
<script src="assets/js/flickity.pkgd.min.js"></script>
<script type="text/javascript">
    // change header image after 2 seconds
    var images = ['second_banner.jpg', 'first_banner.jpg'],
        index = 0, // starting index
        max_images = images.length - 1;
    var timer = setInterval(function () {
        var current_image = images[index];
        index = (index == max_images) ? 0 : ++index;
        $('.main_header_image').fadeOut(300, function () {
            $('.main_header_image').attr("src", 'assets/pics/' + current_image);
            $('.main_header_image').fadeIn(300);
        });
    }, 7000);

    var flkty = new Flickity('.main-gallery', {
        cellAlign: 'left',
        contain: true,
        wrapAround: true,
        prevNextButtons: false,
        autoPlay: 5000
    });
    $('.suscribe').unbind('click').click(function (e) {
        e.preventDefault();
        $('.subscribe_btn').addClass('disabled');
        let email_to_send = $('.email_to_subscribe').val();
        if (email_to_send != '') {
            send_welcome_email(email_to_send, function () {
                $('#subscribe_to').trigger('reset');
                $('.modal_container').removeClass('hide');
                $('.subscribe_btn').removeClass('disabled');
            });
        } else {
            alert('Por favor ingresa tu correo.')
        }
    });

    function send_welcome_email(new_email, callback) {
        $.post("sending.php", {new_email: new_email}).done(
            function (data) {
                callback();
            }
        );
    }

    $(document).ready(function(){
        $('.close_modal_subscribe').unbind('click').click(function(){
            $('.modal_container').addClass('hide');
        });
    });
</script>
<!-- Google Tag Manager (noscript) -->
<noscript>
    <iframe src="https://www.googletagmanager.com/ns.html?id=GTM-PWWRSQ9" height="0" width="0"
            style="display:none;visibility:hidden"></iframe>
</noscript>
<!-- End Google Tag Manager (noscript) -->
</body>
</html>
